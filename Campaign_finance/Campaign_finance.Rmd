...
## I am also curious how much crossover there is between Hogan and Schulz donors. I am going to filter the Hogan dataset for his 2018 annual data, which was the final fundraising push before his run for a second term in that year. Annoyingly, there is an extra space between "2018" and "Annual" in the set, so I am going to fix that first.
hogan_data <- hogan_data %>%
  mutate(`Filing Period` = case_when(`Filing Period` == "2018  Annual" ~ "2018 Annual",
                                     TRUE ~ `Filing Period`))
hogan_data <- hogan_data %>%
  filter(`Filing Period` == "2018 Annual")

## Just curious, how much did Hogan raise that year?
hogan_data %>%
  summarise(total = sum(`Contribution Amount`)) ##almost $5.5 million

## Now I want to join the Hogan and Schulz datasets to see how many overlapping donors there are
hogan_schulz_joined <- hogan_data %>%
  inner_join(schulz_data, by=c("Contributor Name"))

## some of these results are duplicated because people that donated to Schulz once also donated to Hogan more than once, and vice versa. I want to make it so that I only see unique donations to Schulz to calculate the amount of money.
hogan_schulz_joined <- hogan_schulz_joined %>%
  distinct()
...
## There were 377 people or entities that contributed to Hogan's 2018 campaign that also contributed to Schulz' campaign. 

hogan_schulz_joined %>%
  summarise(total = sum(`Contribution Amount.y`))

## This amounted to $261k for the Schulz campaign. 
...
## That is just under 20% of all contributors to her campaign. 
``` 
